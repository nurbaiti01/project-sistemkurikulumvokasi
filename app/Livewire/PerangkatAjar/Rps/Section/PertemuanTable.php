...
class PertemuanTable extends Component
{
    ...
    {
        $data = collect($this->pertemuans)
            ->map(
                fn($pertemuan) => collect($pertemuan)
                    ->except([
                        'show',
                        'selected_bobot_index',
                    ])
                    ->toArray()
            )
            ->values()
    ...
    {
        return view('livewire.perangkat-ajar.rps.section.pertemuan-table');
    }
}
